...
│   ├── 05-modules/                   modules.md            (1 batch file, run once per module, covers 11 documents)
│   ├── 06-development/                 development.md       (1 batch file, two internal waves, covers 10 documents)
│   └── 07-cross-cutting/                 cross-cutting.md      (1 batch file, generated LAST, covers 2 documents)
...
├── 7-sprint-planning/        select the next sprint's work, retro the last one
├── 8-implementation/         implement, review, test, one task at a time
├── 9-sync-docs/              bring docs-kit back in line with what was actually built, then a per-epic design/data-flow/docs-vs-code review
...
| `3-document-generate/05-modules/` | `modules.md` (1 batch file, 11 documents per module) | Draft all 11 documents for every module in `5-modules/`, one module per run. Waits on `02`/`03`/`04`. |
| `3-document-generate/06-development/` | `development.md` (1 batch file, 10 documents, **two waves**) | Draft `6-development/` in two batch runs: early wave (6 documents) parallel with `02`/`03`/`04`; late wave (4 documents, `5-implementation-workflow.md` on) waits on `05-modules/` finishing every module. |
| `3-document-generate/07-cross-cutting/` | `cross-cutting.md` (1 batch file, 2 documents) | Draft all of `7-cross-cutting/` in one run. Always **last** — cross-checks decisions made in every other category. |
...
| `4-document-review/` | `2-documentation-review.md` | Final full-tree consistency sweep. Run **once**, after every category/module's batch has been through `1-document-review.md` and is approved. |
...
| `6-implementation-plan/` | `1-implementation-plan.md` | Derive Milestones → Epics → Tasks from the now-current `docs-kit/`. |
| `7-sprint-planning/` | `1-sprint-planning.md` | Determine the active milestone, select the next sprint's tasks from its epics. Run once per sprint. |
...
| `8-implementation/` | `1-implement-task.md` | Implement the next task in the current sprint. Run once per task. |
...
| `9-sync-docs/` | `2-module-completion-review.md` | Design-fidelity + cross-module data-flow + docs-vs-code consistency check for one epic. Run once per epic, after all its tasks are `Done`, before the epic can be marked `Complete`. |
...
Every document-generation prompt in this library can leave behind a `[NEEDS INPUT: ...]` marker (something unclear that couldn't be answered) or a `[Assumption: ...]` marker (a reasonable guess made instead of guessing silently). Leaving these buried in a file and just continuing to the next prompt means the developer only discovers them by chance — usually too late, after something downstream was already built on top of the gap.
...
- **Documentation generation loop** (per category/module in the plan): `3-document-generate/<category>/<batch-file>.md` → `4-document-review/1-document-review.md` (scoped to that whole category/module's batch) → repeat for the next category/module. `02-database/`, `03-api/`, `04-ui/` (and `06-development/`'s early wave) have no dependency on each other and can be worked in any order once `01-project/` is approved; `05-modules/modules.md` runs once per module once `02`/`03`/`04` are done; `06-development/development.md`'s late wave waits until every module is done; `07-cross-cutting/cross-cutting.md` is always last, after everything else. Once every planned document across every category/module is approved, run `4-document-review/2-documentation-review.md` once, then `5-update-sot/1-update-sot.md` once.
- After that, run `6-implementation-plan/1-implementation-plan.md` once to establish Milestones/Epics/Tasks.
- **Sprint loop**: `7-sprint-planning/1-sprint-planning.md` (once per sprint) → **implementation loop** (per task in that sprint): `8-implementation/1-implement-task.md` → `2-code-review.md` → `3-generate-tests.md` → `9-sync-docs/1-sync-docs.md` → back to `8-implementation/1-implement-task.md` for the next task in the sprint. Once every task in an epic is `Done`, run `9-sync-docs/2-module-completion-review.md` once for that epic before it can be marked `Complete` in `epics.md`. Once the sprint's tasks are all `Done`, run `7-sprint-planning/2-retrospective.md`, then go back to `1-sprint-planning.md` to plan the next sprint — it will tell you to run `10-release/1-release.md` instead if the milestone is actually `Complete` (which itself requires every epic's `9-sync-docs/2-module-completion-review.md` to have passed, not just its tasks being `Done`).
