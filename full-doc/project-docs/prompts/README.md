...
│   │                         between them); 05-modules loops over every module, upfront, once 01-04 and 06's
│   │                         early wave are approved — see "Upfront module documentation" below; 07 runs last.
...
│   ├── 05-modules/                   modules.md            (1 batch file, one module per run, loops upfront over every module, covers 11 documents)
│   ├── 06-development/                 development.md       (1 batch file; early wave upfront, late wave once per module in the upfront loop)
│   └── 07-cross-cutting/                 cross-cutting.md      (1 batch file, generated LAST, after the module loop finishes — covers 2 documents)
...
├── 7-sprint-planning/        select the next sprint's work (pure task-picker — every module's docs and tasks already exist by this point), retro the last sprint
...
├── 11-dashboard/               generate a static HTML progress view (documentation-generation status, plus the full Milestone→Epic→Task→Todo tree); on-demand and auto-triggered on status change
└── 12-maintenance/            on-demand, POST-LAUNCH ONLY: triage an incoming item, scoped new-feature discovery, routine upkeep
```

## Upfront module documentation

`full-doc` generates **all** module documentation before any sprint planning or implementation starts — `3-document-generate/05-modules/modules.md` and `06-development/development.md`'s late wave run in an upfront **module loop**, right after the four global categories (`1-project`, `2-database`, `3-api`, `4-ui`) and `6-development`'s early wave are approved. (This is the opposite of the `on-demand-module-doc` sibling variant, which defers module docs to just-in-time, per sprint — don't mix instructions between the two; confirm which one a given `project-docs/` came from before proceeding.)

**The module loop**, once the four global categories + early wave are approved: for each module in `claude-docs/analysis/module-list.md`, in dependency order per `claude-docs/plan/dependencies.md` — `0-field-extraction.md` (if needed) → `05-modules/modules.md` (11 docs) → review → promote → `06-development/development.md` late wave (scoped to this module) → review → promote → `5-update-sot/1-update-sot.md` (fold this module in) → move to the next module. Repeat until every module in the list is documented.

Once the loop finishes, `07-cross-cutting/cross-cutting.md` runs — always last, since its two documents cross-check decisions made everywhere else, including every module. Then `6-implementation-plan/1-implementation-plan.md` derives the full task list for every module's epics at once. Only then does `7-sprint-planning/1-sprint-planning.md` start — by that point every epic already has approved docs and real tasks, so sprint planning is a pure task-picker with no documentation-generation gate of its own.
...
| `1-discovery/` | `4-design-creation.md` | **Conditional.** Only if `design-source.md`'s `Source:` is `None` (or blank) and the user wants a real design instead of generic defaults — produces `sot-docs/design/tokens.json` from scratch. |
...
| `3-document-generate/05-modules/` | `modules.md` (1 batch file, 11 documents per module) | Draft all 11 documents for one module. **Upfront module loop**: runs once per module, in dependency order, for every module in `module-list.md` — right after `01-04` and `06`'s early wave are approved, before any sprint or implementation starts. |
| `3-document-generate/06-development/` | `development.md` (1 batch file, 10 documents, **two waves**) | Early wave (6 documents) runs once, upfront, parallel with `02`/`03`/`04`. Late wave (4 documents, `5-implementation-workflow.md` on) runs once per module, scoped and updated, immediately after that module's `05-modules/modules.md` is approved, inside the same upfront module loop. |
| `3-document-generate/07-cross-cutting/` | `cross-cutting.md` (1 batch file, 2 documents) | Draft all of `7-cross-cutting/` in one run. Always **last** — cross-checks decisions made in every other category, including every module. Waits until the module loop finishes documenting every module in `module-list.md`. |
...
| `4-document-review/` | `2-documentation-review.md` | Final full-tree consistency sweep. Run **once**, after every category/module's batch has been through `1-document-review.md` and is approved, and the module loop has finished every module in `module-list.md`. |
...
| `6-implementation-plan/` | `1-implementation-plan.md` | Derive Milestones → Epics → the full Task list from `module-list.md` and every approved `docs-kit/` category, including every module — runs once, after the module loop and `07-cross-cutting/` are both done, deriving real tasks for every epic in one pass (no empty/TBD placeholder task lists). |
| `7-sprint-planning/` | `1-sprint-planning.md` | Determine the active milestone, then select the sprint's tasks — every epic already has approved docs and a real task list by this point, so this is a pure task-picker. Run once per sprint. |
...
| `11-dashboard/` | `1-generate-dashboard.md` | Regenerates `claude-docs/plan/dashboard.html` (stat tiles, documentation progress per category/module, a sprint plan table, the full drillable Milestone→Epic→Task→Todo breakdown, and a blockers section) from the current plan/drafts/review-log/sprint/todo files. Markdown-sourced only — never reads actual application source code. Runs on demand any time, **and** auto-triggers right after any Task/Epic/Sprint/Milestone status change in `8-implementation/1-implement-task.md`, `2-code-review.md`, `9-sync-docs/2-module-completion-review.md`, or `7-sprint-planning/1-sprint-planning.md` — not per-Todo tick. |
...
- **`06-development/development.md`'s two waves** — the late wave needs that module's own `05-modules/modules.md` approved first (a real per-module dependency, not a batch-internal stop), so early wave and each module's late-wave run are necessarily separate confirmed runs of the same file, not a mid-batch stop.
...
At the start of a new session in a project already using this kit (not a brand-new one), before asking "what would you like to do," check `project-docs/claude-docs/plan/milestone-status.md`, `epics.md`, the current `project-docs/claude-docs/sprints/sprint-{{n}}.md`, and the latest entry in `project-docs/claude-docs/plan/session-log.md` if it exists (see "Session-end checkpoint" below), and give a short plain-language recap of where things stand — active milestone, current sprint, what's in progress/blocked, and what the last session's closing note said to pick up next — so the developer isn't the one who has to reconstruct state or go open `dashboard.html` themselves before the conversation can start. Skip this only if the state files don't exist yet (brand-new project, nothing planned).

## Session-end checkpoint

The recap above reconstructs *what* state things are in from the tracking files (status fields, drafts present, tasks claimed) — but it can't reconstruct *why* a session stopped where it did, or what the next session should actually pick up first, since that reasoning only ever existed in the closing conversation. Before ending any session that did real work (drafted/reviewed a document, implemented/reviewed/tested a task, planned a sprint — not a purely read-only or Q&A session), append one short entry to `project-docs/claude-docs/plan/session-log.md` (create it with a header row if it doesn't exist yet):

- **Date**
- **What got done** this session, one line
- **Why stopped here** — a natural boundary (per Session Boundaries below), or something else worth naming (blocked on a question, context climbing past 150k, developer had to leave)
- **What's next, specifically** — not just "continue," but the actual next prompt/task, and anything the next session should know before starting it (an answer given verbally but not yet written to a file — see "chance to lose content" — belongs here if it happened right at session close)

This is a few seconds of writing, not a new prompt to run — do it as part of wrapping up, the same way you'd already be stating "next step" to the developer per "Reporting back after a prompt runs" above; just also write it here so it survives past this conversation. Skip it for sessions that were pure discussion/Q&A with no file changes — nothing to check-point. Keep each entry compact — a few short lines per field, not a paragraph; this is a working log re-read at the start of every future session, not a deliverable.
...
**Implementation.** Default split point is between tasks — after a task's commit and status update (`8-implementation/1-implement-task.md` step 11), before claiming the next one. **`2-code-review.md` and `3-generate-tests.md` each run in their own new session — implement, review, and test are three separate sessions, never the same one carried forward.** This is an independence rule, not just a size boundary: the conversation that just wrote the code carries the same blind spots reviewing it or writing its tests (a test written by the same reasoning that produced a bug can just as easily confirm the bug instead of catching it) — a fresh session with no memory of writing it gives a genuinely independent look at each stage, at no extra cost since continuity already comes from files, not conversation memory. If tasks are small and numerous, split between sprints instead (after `7-sprint-planning/2-retrospective.md`) to cut down on how often a session restarts — but the implement/review split itself is never skipped, regardless of task size. A milestone is the outer wrapper only, never the split unit itself — Milestone 2/3+ span many sprints, so waiting for a milestone boundary means the session has already grown large by the time it splits. Never split mid-task (uncommitted code, a half-checked Todo list) or mid design-review loop (`8-implementation/1-implement-task.md`'s Module Design-First Strategy) — that state exists only in the conversation, a fresh session can't reconstruct it from files.
...
- **Upfront documentation generation loop, global categories** (per category in the plan): `3-document-generate/<category>/<batch-file>.md` → `4-document-review/1-document-review.md` (scoped to that category's batch) → repeat for the next category. `02-database/`, `03-api/`, `04-ui/`, and `06-development/`'s early wave have no dependency on each other and can be worked in any order once `01-project/` is approved. Once `01-project`, `02-database`, `03-api`, `04-ui`, and `06-development`'s early wave are all approved, run `5-update-sot/1-update-sot.md` once.
- **Upfront module loop** (see "Upfront module documentation" above): for each module in `module-list.md`, in dependency order — `0-field-extraction.md` (if needed) → `3-document-generate/05-modules/modules.md` (scoped to that module) → `4-document-review/1-document-review.md` (scoped to that module) → `3-document-generate/06-development/development.md` late wave (scoped to that module) → `4-document-review/1-document-review.md` (scoped to that late-wave slice) → `5-update-sot/1-update-sot.md` (folds this module into the SoT) → move to the next module in the list. Repeat until every module is documented.
- **Cross-cutting and implementation planning:** once the module loop finishes every module, run `3-document-generate/07-cross-cutting/cross-cutting.md` → `4-document-review/1-document-review.md` (scoped) → `4-document-review/2-documentation-review.md` (final full-tree sweep, once) → `5-update-sot/1-update-sot.md` (once, its final cycle). Then run `6-implementation-plan/1-implementation-plan.md` once — every module's docs already exist, so it derives the full Milestone/Epic/Task set for the whole project in one pass, no empty/TBD placeholder task lists.
- **Sprint loop**: `7-sprint-planning/1-sprint-planning.md` (once per sprint) — pure task-picker at this point, every epic already has approved docs and real tasks. Once the sprint's task list is finalized, go straight to the **implementation loop** (per task in that sprint): `8-implementation/1-implement-task.md` → `2-code-review.md` → `3-generate-tests.md` → `9-sync-docs/1-sync-docs.md` → back to `8-implementation/1-implement-task.md` for the next task in the sprint. Once every task in an epic is `Done`, run `9-sync-docs/2-module-completion-review.md` once for that epic — it can only mark the epic `Complete` in `epics.md` once its own automated checks pass **and** the user has explicitly confirmed the module's real functionality live in a browser (fix-looping on anything they report wrong first). Once the sprint's tasks are all `Done`, run `7-sprint-planning/2-retrospective.md`, then go back to `1-sprint-planning.md` to plan the next sprint — it will tell you to run `10-release/1-release.md` instead if the milestone is actually `Complete`. Every Task/Epic/Sprint/Milestone status change through this whole loop also auto-triggers an `11-dashboard/1-generate-dashboard.md` refresh (not per-Todo) — see that prompt's table row above.
- **On-demand, outside the linear sequence:** `10-release/2-incident-response.md` runs any time production breaks; `1-discovery/7-change-request.md` runs any time a requirement changes after docs/tasks already exist; `11-dashboard/1-generate-dashboard.md` runs any time the user wants an updated HTML progress view (also auto-triggered by status changes elsewhere in the flow — see its table row above); `12-maintenance/1-triage.md` runs any time a new item comes in post-launch. All four regardless of what else is in progress.
