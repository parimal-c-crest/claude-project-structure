...
## Next Step
`4-ui/` drafts are complete. Run `project-docs/prompts/4-document-review/1-document-review.md` scoped to `4-ui` next — nothing here is promoted into `approved-docs/docs-kit/` until it does. Once `4-ui/` is approved, `6-development/`'s early wave can start in parallel if it hasn't already — run `project-docs/prompts/3-document-generate/06-development/development.md` (early wave). Once `01-project`, `02-database`, `03-api`, `04-ui`, and `06-development`'s early wave are **all** approved, the upfront module loop starts: `project-docs/prompts/3-document-generate/05-modules/modules.md`, scoped to the first module in `module-list.md` (dependency order per `dependencies.md`) — every module's full documentation gets generated now, before any sprint planning or implementation starts. See `project-docs/prompts/README.md`'s "Upfront module documentation" section for the full loop.
