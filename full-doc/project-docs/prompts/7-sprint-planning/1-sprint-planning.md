...
## Objective
Determine the active milestone, then select, prioritize, and commit a specific set of tasks from that milestone's epics to the upcoming sprint, respecting dependencies and capacity. Under this variant's upfront module documentation flow, every module's docs and task list already exist by the time sprint planning ever runs (generated by the module loop in `3-document-generate/05-modules/modules.md` before implementation started) — this prompt is a pure task-picker, with no documentation-generation gate of its own.
