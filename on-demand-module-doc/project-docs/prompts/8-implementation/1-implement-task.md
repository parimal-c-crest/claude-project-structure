...
4. Break `{{task_id}}` into a Todo checklist — the concrete steps needed to complete it (typically 3–8 items, e.g. "add migration", "add model", "add endpoint", "wire up validation"). Write this to `project-docs/claude-docs/tasks/{{task_id}}-todos.md` before writing code, with a one-line header tracking progress, e.g. `Status: 0/5 complete`. Keep each item terse (a short label, not a sentence) — this file is a working checklist re-read every session touching this task, not a deliverable read outside the workflow.
...
## Next Step
Run `project-docs/prompts/8-implementation/2-code-review.md` next, on this task — **in a new session, not this one.** This is a review-independence rule, not just a size boundary (see `prompts/README.md`'s "Session Boundaries" → "Implementation"): a fresh session with no memory of writing this code reviews it more honestly than the same conversation that just wrote it.
