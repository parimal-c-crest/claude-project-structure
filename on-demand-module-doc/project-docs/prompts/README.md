...
├── 7-sprint-planning/        select the next sprint's work (including the JIT module-doc gate, step 2a), retro the last sprint
...
- **What's next, specifically** — not just "continue," but the actual next prompt/task, and anything the next session should know before starting it (an answer given verbally but not yet written to a file belongs here if it happened right at session close)
